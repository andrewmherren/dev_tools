...
3. **Create storage directories**:
   ...
   ```bash
   mkdir -p /absolute/path/to/dev_tools/.server_data/.ollama
   mkdir -p /absolute/path/to/dev_tools/.server_data/.sonarqube
   ```
   ...
   ```powershell
   New-Item -Path "<absolute-path>/dev_tools/.server_data/.ollama" -ItemType Directory -Force
   New-Item -Path "<absolute-path>/dev_tools/.server_data/.sonarqube" -ItemType Directory -Force
   ```
...
For comprehensive instructions on configuring your projects to use this dev server, see **[PROJECT_SETUP.md](PROJECT_SETUP.md)**.
